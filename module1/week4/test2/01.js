...
const keyMatcher = function(firstObj, secondObj, key) {
  ...
  if (findKey(firstObj) === findKey(secondObj)) {
    return true;
  } else if (findKey(firstObj) !== findKey(secondObj)) {
    return false;
  }
};

console.log(keyMatcher({a: 1, b: 2, c: 3}, {here: 3, is: 2, a: 1, silly: 0, example: -1}, 'a')); //returns true (since the value and type are the exact same)
console.log(keyMatcher({apple: "red", banana: "yellow", cherry: "red"}, {apple: "green", banana: "brown", cherry: "black"}, "apple")); // returns false since the values are completely different ("red" vs "green")
console.log(keyMatcher({a: 1, b: 2, c: 3}, {a: "1", b: "2", c: "3"}, 'c')); // returns false since the values are different types (3 vs "3")
console.log(keyMatcher({a: 1, b: 2, c: 3}, {d: 4, e: 5, f: 6}, 'b')); // returns false since b is not in the second object
