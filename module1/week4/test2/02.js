...
console.log(countWhich([1, 2, 3, 4, 5], function(num) {
  return (num > 4);
})); // returns 1 (only matches 5)
console.log(countWhich(["apple", "banana", "cherry"], function(fruit) {
  return fruit[0] === "a";
})); // returns 1 (only matches apple)
console.log(countWhich([10, 20, 30, 40, 50], function(num) {
  return num % 7 === 0;
})); // returns 0 (none of the numbers are divisible by 7)
console.log(countWhich(["apple", "banana", "cherry"], function(fruit) {
  return fruit.length > 5;
})); // returns 2 ("apple" is shorter than 6 characters)
console.log(countWhich([], function(x) {
  return x > 10;
})); // returns 0
console.log(countWhich("This should fail", function(word) {
  return true;
})); // returns false (because the first argument is not an array)
